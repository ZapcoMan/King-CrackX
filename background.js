 ...
 * @returns {Object<string, true>} 归一化后的白名单，形如 { "example.com": true }
 */
 ...
 * 由调用方通过 falsy 判断过滤掉。
 *
 * @param {string} siteKey - 待归一化的站点标识
 * @returns {string} 小写 hostname；不合法时为空字符串
 */
 ...
 *
 * @param {string} siteKey - 站点标识，内部会先做归一化
 * @returns {string} match pattern；输入不合法时为空字符串
 */
 ...
 *
 * @returns {Promise<chrome.scripting.RegisteredContentScript|null>} 已注册的脚本对象；未注册时为 null
 */
 ...
 *
 * @param {string[]} matches - match pattern 列表，为空时直接返回不注册
 * @returns {Promise<void>}
 */
 ...
 * 保证任意时刻只有一个同步流程在跑。
 *
 * @returns {Promise<void>} 当前队列尾部的 Promise，可用于等待本轮同步结束
 */
