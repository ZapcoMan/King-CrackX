...
(function () {
    ...
    let detectionResult: VueDetectionResult = {
        detected: false,
        method: '',
        details: {},
        errorMsg: ''
    ...
    let routerAnalysisResult: RouterAnalysisResult | null = null;
     ...
     *
     * @returns true 表示已发起注入；false 表示被并发保护拦截或注入失败
     */
    function injectDetector(): boolean {
        ...
        try {
            ...
            detectionResult.errorMsg = String(e);
     ...
     *
     * @returns true 表示已发起注入；false 表示注入失败
     */
    function injectApiExtractor(): boolean {
        try {
            const script = document.createElement('script');
    ...
    window.addEventListener('message', function (event: MessageEvent) {
        ...
        try {
            const data = event.data as PageScriptMessage;

            // ---- 类型 1：Vue 检测结果 ----
            if (data.type === 'VUE_DETECTION_RESULT') {
                detectionResult = data.result as unknown as VueDetectionResult;
            ...
            // ---- 类型 1：路由分析结果 ----
            else if (data.type === 'VUE_ROUTER_ANALYSIS_RESULT') {
                routerAnalysisResult = data.result as RouterAnalysisResult;
                // 分析流程结束，释放注入锁
                detectorInjectionInProgress = false;
            ...
            // ---- 类型 1：路由分析出错 ----
            else if (data.type === 'VUE_ROUTER_ANALYSIS_ERROR') {
                detectorInjectionInProgress = false;
                chrome.runtime.sendMessage({
            ...
            // ---- 类型 2：梭哈模式状态上报 ----
            else if (data.type === 'VUECRACK_ALL_IN_STATUS' && data.source === 'vuecrack-all-in') {
                chrome.runtime.sendMessage({
            ...
            // ---- 类型 3：API 提取进度 ----
            else if (data.type === 'VUECRACK_API_EXTRACT_PROGRESS' && data.source === 'vuecrack-api-extractor') {
                chrome.runtime.sendMessage({
            ...
            // ---- 类型 3：API 提取结果 ----
            else if (data.type === 'VUECRACK_API_EXTRACT_RESULT' && data.source === 'vuecrack-api-extractor') {
                chrome.runtime.sendMessage({
            ...
            // ---- 类型 3：API 提取出错 ----
            else if (data.type === 'VUECRACK_API_EXTRACT_ERROR' && data.source === 'vuecrack-api-extractor') {
                chrome.runtime.sendMessage({
                    action: "vuecrackApiError",
                    error: data.error
                });
     ...
     */
    chrome.runtime.onMessage.addListener(function (request: PopupRequest, _sender, sendResponse) {
        try {
