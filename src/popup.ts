/**
 * King-CrackX —— popup 界面逻辑
 *
 * 职责：
 *   1. 触发并展示 Vue Router 分析结果（路由枚举 → 生成可点击的完整 URL）
 *   2. 管理「梭哈模式」的站点白名单开关，并展示实时拦截统计
 *   3. 触发 API 端点提取并展示 / 复制 / 导出结果
 *
 * 数据来源全部为 content.js 通过 chrome.runtime.sendMessage 主动推送，
 * 因此本文件主要负责「发指令 + 渲染」，不依赖 sendMessage 的返回值。
 */
(function () {
    ...
    const allInToggle = document.getElementById('allInToggle') as HTMLInputElement | null;
    ...
    /** 全局变量存储路由分析结果 */
    let vueAnalysisResult: RouterAnalysisResult | null = null;
    let apiExtractResult: ApiExtractResult | null = null;
    ...
    let latestAllInStatus: AllInStatus | null = null;
    let allInSiteKey = '';
    let allInModeInitialized = false;
    const basePathModeByOrigin: Record<string, string> = {};
    ...
    function urlsEqual(left: string | undefined, right: string | undefined): boolean {
    ...
    function escapeHtml(value: unknown): string {
        ...
        return String(value ?? '').replace(/[&<>"']/g, char => charMap[char]);
    }
    ...
    function getBaseModeKey(url: string): string {
        try {
    ...
    function splitUrlForDisplay(url: string): { domain: string; path: string } {
        try {
    ...
    function getAllInSiteKey(url: string | undefined): string {
        try {
    ...
    function pruneAnalysisCache(): void {
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                try {
                    const parsed = JSON.parse(localStorage.getItem(key) as string) as CachedAnalysis | null;
                    entries.push({
                        key,
    ...
    function pruneRouteMemory(): void {
        try {
            const entries: Array<{ key: string; savedAt: number }> = [];
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                try {
                    ...
                    const parsed = rawValue ? JSON.parse(rawValue) as { savedAt?: number } | null : null;
                    entries.push({
                        key,
                        savedAt: parsed?.savedAt || 0
                    });
    ...
    function readCachedAnalysis(url: string | undefined): CachedAnalysis | null {
    ...
    function writeCachedAnalysis(url: string | undefined, result: RouterAnalysisResult | null): void {
        const normalizedUrl = normalizeUrl(url);
    ...
    function clearCachedAnalysis(url: string | undefined): void {
        const normalizedUrl = normalizeUrl(url);
        if (!normalizedUrl) {
            return;
        }
    ...
    function writeLastOpenedRoute(pageUrl: string | undefined, routeUrl: string | undefined): void {
        if (!pageUrl || !routeUrl) {
            return;
        }
    ...
    function readLastOpenedRoute(pageUrl: string | undefined): string {
        if (!pageUrl) {
            return '';
        }
    ...
    function dedupeUrlItems(items: UrlItem[]): UrlItem[] {
    ...
    function normalizeAnalysisResult(result: RouterAnalysisResult | null | undefined): RouterAnalysisResult {
    ...
    function ensureActiveRouteVisible(activeItem: Element | null): void {
        if (!activeItem) {
            return;
        }
    ...
    function setAllInStatusText(message: string): void {
        if (allInStatus) {
            allInStatus.textContent = message;
        }
    }

    function renderAllInStatus(): void {
        if (!allInEnabled) {
    ...
    function initializeAllInMode(pageUrl: string | undefined = currentTabUrl): void {
        if (!allInToggle) {
            return;
        }
    ...
    /** 安全的错误处理 */
    function safeExecute<T>(fn: () => T, context: string = 'Unknown'): T | null {
        try {
            return fn();
        } catch (error) {
            console.error(`Error in ${context}:`, error);
            const message = error instanceof Error ? error.message : String(error);
            showError(`${context}执行出错: ${message}`);
            return null;
        }
    }

    // ===== API端点提取 =====
    function showApiLoading(message?: string): void {
    ...
    function requestApiExtraction(): void {
        const tabId = currentTabId;
        if (!tabId) {
        ...
        chrome.tabs.sendMessage(tabId, { action: "extractApi" }, function () {
            if (chrome.runtime.lastError) {
    ...
    function bindApiResultActions(): void {
        const copyFullBtn = document.getElementById('apiCopyFullBtn');
        if (copyFullBtn) {
            copyFullBtn.addEventListener('click', function (this: HTMLElement) {
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', function (this: HTMLElement) {
                try {
        ...
        if (copyJsonBtn) {
            copyJsonBtn.addEventListener('click', function (this: HTMLElement) {
        ...
        document.querySelectorAll('.api-copy-item-btn').forEach(btn => {
            btn.addEventListener('click', function (this: Element) {
                ...
                }).catch(() => { /* 复制失败时静默，保持原按钮文案 */ });
            });
        });
    }

    function collectAllApiPaths(): string[] {
        const paths: string[] = [];
    ...
    function collectAllApiFullUrls(): string[] {
        const urls: string[] = [];
    ...
    function collectUncalledApiUrls(): string[] {
        if (!apiExtractResult) return [];
    ...
    /** 生成TXT导出内容 */
    function buildApiExportText(): string {
        if (!apiExtractResult) return '';
    ...
    function buildApiExportFilename(): string {
        let host = 'site';
        try {
            host = new URL(currentTabUrl).hostname.replace(/[^a-zA-Z0-9.-]/g, '_');
        } catch (error) { /* 当前标签页 URL 非法时退回默认名 site */ }
    ...
    function displayApiResult(result: ApiExtractResult): void {
    ...
    function showLoading(message: string): void {
        if (!routerAnalysisContainer) return;
    ...
    function updateCurrentTabInfo(callback?: (tab: chrome.tabs.Tab) => void): void {
    ...
    function restoreCachedAnalysis(url: string | undefined): boolean {
    ...
    function requestCurrentTabAnalysis({ preserveUi = false, forceRefresh = true }: { preserveUi?: boolean; forceRefresh?: boolean } = {}): void {
        const tabId = currentTabId;
        if (!tabId) {
            showError("无法获取当前标签页信息");
            return;
    ...
    /** 显示Vue检测结果 */
    function displayDetectionResult(result: VueDetectionResult): void {
        if (result.detected) {
    ...
    /** 显示Vue Router分析结果 */
    function displayRouterAnalysis(result: RouterAnalysisResult): void {
        safeExecute(() => {
    ...
    function addEventListeners(): void {
        safeExecute(() => {
            ...
            if (copyUrlsBtn) {
                copyUrlsBtn.addEventListener('click', function (this: HTMLElement) {
                    const displayedUrls = document.querySelectorAll('.displayed-urls .url-text');
                    const urlsToUse = Array.from(displayedUrls).map(el => el.textContent as string);
            ...
            pathModeBtns.forEach(btn => {
                btn.addEventListener('click', function (this: Element) {
                    ...
                    if (!modeKey || !['standard', 'base'].includes(mode as string)) {
                        return;
                    }

                    basePathModeByOrigin[modeKey] = mode as string;
    ...
    /** 显示URL列表 */
    function displayUrlList(routes: RouteEntry[] | RouterAnalysisResult | null): void {
        if (!routes) {
            if (pathListContainer) {
        ...
        if (Array.isArray(routes)) {
            ...
            routeArray = keys.map(key => {
                const route = (routes as unknown as Record<string, any>)[key];
                return {
                    path: route.path || key,
                    name: route.name || key
                };
        ...
        updateCurrentTabInfo(function (tab) {
            ...
            `;
                ...
                setTimeout(() => {
                    ...
                    const activeItem = pathListContainer
                        ? pathListContainer.querySelector('.full-url-item.current-route')
                        : null;
    ...
    chrome.runtime.onMessage.addListener(function (request: RuntimeMessage, sender) {
        safeExecute(() => {
            ...
            if (request.action === "vueDetectionResult") {
                displayDetectionResult(request.result as VueDetectionResult);
            }
            else if (request.action === "vueRouterAnalysisResult") {
                const normalizedResult = normalizeAnalysisResult(request.result as RouterAnalysisResult);
                ...
                displayApiResult((request.result || {}) as ApiExtractResult);
            }
